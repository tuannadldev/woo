    ...
    ?>
        <script type="text/javascript">
             var ajaxurl = '<?php echo admin_url('admin-ajax.php'); ?>',
                err_bwl_fca_captcha = '<?php _e(' Incorrect Captcha Value!', 'bwl-wpfmfc'); ?>',
                err_bwl_fca_question = '<?php _e(' Write your question. Min length 3 characters !', 'bwl-wpfmfc'); ?>',
                err_bwl_fca_email = '<?php _e(' Valid Email address required!', 'bwl-wpfmfc'); ?>',
                err_bwl_fca_success_msg = '<?php _e(' FAQ successfully added for review!', 'bwl-wpfmfc'); ?>',
                err_bwl_fca_error_msg = '<?php _e(' Unable to add faq. Please try again!', 'bwl-wpfmfc'); ?>';
...
//        $wpfm_faqs = get_post_meta( $product_id, 'wpfm_contents' );
        ...
        if ( $bwpfm_email_status == TRUE ) {
            ...
            $subject = __('New Product FAQ Question!', 'bwl-wpfmfc');
            $edit_bwl_fca_url =  get_admin_url() . "post.php?post&#61;$post_id&#38;action&#61;edit";

            $body = "<p>". __("Hello Administrator", 'bwl-wpfmfc') . ",<br>" . __("A new FAQ question has been submitted for following product.", 'bwl-wpfmfc') . "</p>";         
            $body .= "<h3>" . __("Product:", 'bwl-wpfmfc') .$product_title. "</h3><br />"; 
            $body .= "<p><strong>" . __("Product URL", 'bwl-wpfmfc') . ":</strong> <a href='".$product_url."'>" . $product_url . "</a></p><hr />";  
            $body .= "<h3>" . __("Question Information", 'bwl-wpfmfc') . "</h3><hr />";        
            $body .= "<p><strong>" . __("Question Title", 'bwl-wpfmfc') . ":</strong><br />" . $title . "</p>";    
            
            if( $sender_name!="" ) :
                $body .= "<p><strong>" . __("Submitted By", 'bwl-wpfmfc') . ":</strong><br />" . $sender_name . "</p>";      
            endif;
            $body .= "<p><strong>" . __("FAQ Status", 'bwl-wpfmfc') . ":</strong> " . __("Pending", 'bwl-wpfmfc') . "</p>";
            $body .= "<p><strong>" . __("Review", 'bwl-wpfmfc') . ":</strong> " . $edit_bwl_fca_url . "</p>";
            $body .= "<p>" . __("Thank You!", 'bwl-wpfmfc') . "</p>"; 
