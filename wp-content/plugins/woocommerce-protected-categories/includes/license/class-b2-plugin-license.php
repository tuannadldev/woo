...
if ( ! class_exists( 'Barn2_Plugin_License' ) ) {
	...
	class Barn2_Plugin_License {
				...
				if ( $this->is_api_error( $response ) ) {
					...
					if ( false === $license_data->success ) {
						switch ( $license_data->error ) {
							case 'missing' :
								$error_message	 = __( 'Please enter your license key.', 'wc-cat-protect' );
								break;
							case 'missing_url' :
								$error_message	 = __( 'No URL was supplied for activation.', 'wc-cat-protect' );
								break;
							case 'license_not_activable' :
								$error_message	 = __( 'This license is for a bundled product and cannot be activated.', 'wc-cat-protect' );
								break;
							case 'expired' :
								$error_message	 = sprintf(
									__( 'Your license key expired on %s.', 'wc-cat-protect' ), date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
								);
								break;
							case 'item_name_mismatch' :
							case 'invalid_item_id':
								$error_message	 = __( 'Your license key is not valid for this plugin.', 'wc-cat-protect' );
								break;
							case 'no_activations_left':
								$error_message	 = __( 'Your license key has reached its activation limit.', 'wc-cat-protect' );
								break;
							case 'disabled':
								$error_message	 = __( 'Your license key has been disabled, please contact support,', 'wc-cat-protect' );
								break;
							case 'key_mismatch':
								$error_message	 = __( 'License key mismatch, please contact support.', 'wc-cat-protect' );
								break;
							default :
								$error_message	 = __( 'There was an error activating your license, please try again.', 'wc-cat-protect' );
								$result			 = 'error';
								break;
						}
					} // if license invalid
		...
		public function get_license_key_admin_message() {
			$message			 = __( 'Please enter your license key.', 'wc-cat-protect' );
			$deactivate_button	 = sprintf( '<button type="submit" class="button" name="deactivate_key" value="%1$s" style="margin-left:8px;">%2$s</button>', esc_attr( $this->license_key_option ), __( 'Deactivate', 'wc-cat-protect' ) );

			if ( $this->is_valid() ) {
				$message = sprintf( '<span style="color:green;">✓ %s</span>', __( 'License key successfully activated.', 'wc-cat-protect' ) ) . $deactivate_button;
			} elseif ( $this->is_deactivated() ) {
				$message = __( 'License key successfully deactivated. Click Save to reactivate.', 'wc-cat-protect' );
			} elseif ( $this->failed_deactivation() ) {
				$message = sprintf( '<span style="color:red;">%s</span>', __( 'There was an error deactivating your license key, please try again.', 'wc-cat-protect' ) ) . $deactivate_button;
			} elseif ( $this->is_invalid() ) {
				$message = $this->get_license_error();
				if ( ! $message ) {
					$message = __( 'There was an error activating your license, please try again.', 'wc-cat-protect' );
				}
		...
		private function get_api_error_message( $response ) {
			if ( is_wp_error( $response ) ) {
				...
				return __( 'An error has occurred, please try again.', 'wc-cat-protect' );
			}
		}
