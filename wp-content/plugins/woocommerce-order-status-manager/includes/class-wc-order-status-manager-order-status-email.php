...
class WC_Order_Status_Manager_Order_Status_Email extends WC_Email {
	...
	public function trigger( $order_id ) {
		...
		if ( $order ) {
			...
			if ( 'customer' === $this->type ) {
				$this->recipient = SV_WC_Order_Compatibility::get_prop( $this->object, 'billing_email' );
			}
			...
			$this->replace['order-date']         = date_i18n( wc_date_format(), SV_WC_Order_Compatibility::get_date_created( $this->object )->getTimestamp() );
			$this->replace['order-number']       = $this->object->get_order_number();
			$this->replace['order-status']       = wc_get_order_status_name( $this->object->get_status() );
			$this->replace['billing-first-name'] = SV_WC_Order_Compatibility::get_prop( $this->object, 'billing_first_name' );
			$this->replace['billing-last-name']  = SV_WC_Order_Compatibility::get_prop( $this->object, 'billing_last_name' );
			$this->replace['billing-company']    = SV_WC_Order_Compatibility::get_prop( $this->object, 'billing_company' );
