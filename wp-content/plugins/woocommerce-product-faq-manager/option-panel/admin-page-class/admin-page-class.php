  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
      ...
      if (count($meta) > 0 && is_array($meta) ){
         foreach ($meta as $me){
           //for labling toggles
           $mmm = ( isset($me[$field['fields'][0]['id']]) && $me[$field['fields'][0]['id']] != "") ? $me[$field['fields'][0]['id']]: "Untitled";
           $mmm = (in_array($field['fields'][0]['type'],array('image','file'))? '' : $mmm);
           echo '<div class="at-repater-block">'.$mmm.'<hr><table class="repeater-table" style="display: none;">';
           if ($field['inline']){
...
* @access public
*/
  public function show_field_hiddentext( $field, $meta) {
    $this->show_field_begin( $field, $meta );
    echo "<input type='hidden' class='at-text".(isset($field['class'])? " {$field['class']}": "")."' name='{$field['id']}' id='{$field['id']}' value='".stripslashes($meta)."' size='50' />"."\n";
    $this->show_field_end( $field, $meta );
  }
...
<input class="button" type="button" value="'. __('Get Export','apc').'" id="apc_export_b" />'.'
