		...
		<?php if ( ! $using_single_location ) : ?>
			...
			<div
				...
				<small
					...
					printf( __( 'Enter a postcode or city to search for pickup locations from: %s', 'woocommerce-shipping-local-pickup-plus' ) . ' <span class="change-span">(' . $change . ')</span>', '<em class="pickup-location-current-lookup-area-label">' . $this->get_lookup_area_label() . '</em>' ); ?>
				</small>
		...
		<?php if ( ! $enhanced_search || $using_single_location ) : ?>
			...
			<?php if ( $using_single_location ) : ?>
				...
				<?php if ( wc_local_pickup_plus_shipping_method()->is_per_order_selection_enabled() ) : ?>
					...
					// Handle a very specific occurrence where the following is true:
					// - there's only one pickup location available at checkout
					// - there's only one package in per-order mode
					// - we are applying tax based on the pickup location address, not the customer or the shop address
					if ( wc_local_pickup_plus_shipping_method()->apply_pickup_location_tax() ) {

						// check if there's only one package: if we are here, it _must_ be for pickup even if session data hasn't been set already
						$packages = WC()->shipping()->get_packages();

						if ( 1 === count( $packages )  ) {

							// set this package for pickup with the current chosen location, since we've made sure already it's determined
							wc_local_pickup_plus()->get_session_instance()->set_package_pickup_data( key( $packages ), [
								'handling'           => 'pickup',
								'pickup_location_id' => $chosen_location->get_id(),
							] );
						}
					}
