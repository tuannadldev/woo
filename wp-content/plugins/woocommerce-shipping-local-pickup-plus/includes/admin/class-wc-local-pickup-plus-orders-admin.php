...
class WC_Local_Pickup_Plus_Orders_Admin {
		...
		if ( 'pickup_locations' === $column ) {
			...
			if ( ! empty( $pickup_locations ) ) {
				...
				if ( ! empty( $lost_locations ) ) {
					...
					$pickup_location_names[] = $locations_not_found . ' ' . '<a href="' . esc_url( get_edit_post_link( Framework\SV_WC_Order_Compatibility::get_prop( $order, 'id' ) ) ) . '">' . __( '(see order)', 'woocommerce-shipping-local-pickup-plus' ) . '</a>';
				}
	...
	public function output_order_shipping_item_pickup_data_field( $item_id, $item ) {
		...
		if (      $order instanceof \WC_Order
			...
			$select2selector = Framework\SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ? 'select' : 'input';
