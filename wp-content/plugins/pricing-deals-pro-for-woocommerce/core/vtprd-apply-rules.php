 ...
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 006 ', true ) );
     ...
     if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_role_excl_array']) > 0) {
        if ($this->vtprd_is_role_in_inPop_list_check($i, $k, 'excl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_role_excl_array']) ) {
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Role exclusion found';
 //error_log( print_r(  'vtprd_is_product_in_inPop_group return false 007 ', true ) );
     ...
     if (sizeof($vtprd_rules_set[$i]->buy_group_population_info['buy_group_email_excl_array']) > 0) {
        if ($this->vtprd_is_email_in_inPop_list_check($i, $k, 'excl', $vtprd_rules_set[$i]->buy_group_population_info['buy_group_email_excl_array']) ) {
          $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_participation_msgs'][] = 'Customer Email/Name exclusion found';
    ...
    public function vtprd_is_role_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
     ...
     //error_log( print_r(  'vtprd_is_role_in_inPop_list_check ', true ) ); 
      ...
      if (array_intersect($this->vtprd_get_current_user_roles(), $test_this_array )) { //v1.1.7.2 grpC
        ...
        $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_role_found'] = 'Role foud for '.$incl_or_excl ;    
        return true;                                
      } else {
    ...
    public function vtprd_is_email_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
     ...
     //error_log( print_r(  'vtprd_is_email_in_inPop_list_check ', true ) ); 
      ...
      if (in_array($current_user->ID, $test_this_array)) {
        $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_user_found'] = 'User found for '.$incl_or_excl ;    
        return true;       
      }

      $vtprd_cart->cart_items[$k]->cartAuditTrail[$vtprd_rules_set[$i]->post_id]['inPop_role_found'] = 'User not found for  '.$incl_or_excl;
      return false;
    }
    ...
    public function vtprd_are_memberships_in_inPop_list_check($i,$k, $incl_or_excl, $test_this_array) {
      global $vtprd_cart, $vtprd_rules_set, $vtprd_rule, $vtprd_info, $vtprd_setup_options;      
       if (sizeof($vtprd_cart->cart_items[$k]->memberships_list) == 0) {
          //v2.0.0.8 begin - backwards compatible pre-v1.7
          if (function_exists('wc_memberships_get_user_active_memberships'))  {
            ...
            $vtprd_cart->cart_items[$k]->memberships_list = wc_memberships_get_user_memberships( $user_id, $args );          
          }
          //v2.0.0.8 end
       }
       if (sizeof($vtprd_cart->cart_items[$k]->memberships_list) == 0) {
	...
	public function vtprd_auto_add_to_vtprd_cart($free_product_id, $free_product_to_be_added_qty, $i) {
     ...
     //error_log( print_r(  'TOP OF  auto_add_to_vtprd_cart' , true ) );  
      ...
      if ( (isset($vtprd_info['product_session_info']['product_id'])) &&
        ...
        $unit_price = vtprd_get_current_active_price($free_product_id,$_product);
      }
      //v2.0.0 G Solution End   
