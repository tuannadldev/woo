...
add_shortcode( 'phoen_reward_points_user', 'phoen_reward_points_fun' );

function phoen_reward_points_fun(){
				...
				if(!empty($phoen_reward_referral_user_points))
				{
					for($j=0; $j<count($phoen_reward_referral_user_points); $j++)
					{
						if($phoen_reward_referral_user_id==$cur_id)
						{
							...
							//$add_pall_points = $phoen_reward_referral_user_pointsss+$phoen_first_login_points;
		...
		for($i=0;$i<count($products_order);$i++){	
			...
			if($user_id ==$cur_id)
			{
				...
				$phoen_current_dates = date("d-m-Y") ;
					...
					{
						$phoen_complited_date_exp= $products_detail['_completed_date'][0];
						...
						if($phoen_complited_date_exp!='')
						{
							...
							if($payment_gatway_val_exp !='')
							{
							 ...
							 if($used_reward_point_exp !='' && $used_reward_point_exp!='')
							{
								?>
								<tr>
									<td><?php echo $phoen_complited_date_exp ;?></td>
			...
			if(empty($products_order))
			{
				$current_user = wp_get_current_user();
				$cur_id = $current_user->ID;
				$phoen_current_datse = date("d-m-Y h:i") ;
